...
namespace AssetTracker.AssetsImporter.Steam.Services
{
    public class SteamService : ISteamService
    {
        ...
        {
            var url = $"https://api.steampowered.com/IPlayerService/GetOwnedGames/v1/?key={steamApiKey}&steamid={steamId}&include_appinfo=1&include_played_free_games=1";
        ...
        public async Task<SteamAppData> GetSteamGameDetailsAsync(int appId, CancellationToken cancellationToken = default)
        {            
            ...
            var json = await response.Content.ReadAsStringAsync(cancellationToken);
            var result = JsonSerializer.Deserialize<SteamAppDetailsResponse>(json, new JsonSerializerOptions
            {
                PropertyNameCaseInsensitive = true
            });

            return result?.Data ?? new SteamAppData();
        }
    }
}
